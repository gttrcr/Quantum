...
namespace GnuPlot
{
	class GnuGraph : public GnuGraphFormatter
	{
	public:
		GnuGraph(const std::string& gnuplotExePath = "C://gnuplot//bin//gnuplot.exe")
		{
			_gnuplotExePath = gnuplotExePath;
			_gnuplotPipe = _popen(_gnuplotExePath.c_str(), "w");
			if (!_gnuplotPipe)
